...
parser.add_argument('--ask',required=True); parser.add_argument('--role',choices=ROLE_CLEARANCE,default='internal_auditor')
parser.add_argument('--backend',choices=['chroma','pinecone','json'],default=None); args=parser.parse_args()
...
hits=retrieve(args.ask,records,top_k=5,backend=args.backend,allowed_sources=sorted({r['source'] for r in records})); STORE_DIR.mkdir(exist_ok=True)
with (STORE_DIR/'audit_log.jsonl').open('a',encoding='utf-8') as f: f.write(json.dumps({'time':datetime.now().isoformat(timespec='seconds'),'role':args.role,'question_hash':__import__('hashlib').sha256(args.ask.encode()).hexdigest(),'sources':[h['source'] for h in hits]},ensure_ascii=False)+'\n')
